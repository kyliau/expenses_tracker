...
class Expense(ndb.Model):
	...
	paid_by = ndb.StringProperty(indexed=False)

class ExpenseEntry:
	def __init__(self, item, amount, paid_by):
 		self.item = item
 		self.amount = amount
...
class MainPage(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        expense.item = self.request.get('item')
        expense.amount = self.request.get('amount')
        expense.paid_by = self.request.get('paid_by')
        expense.put()
    ...
    def get(self):
        ...
        for expense in expenses:
        	expense_entry = str(expense.date) + ' ' + expense.item + ' ' + str(expense.amount) + ' ' + expense.paid_by
        	self.response.write('<blockquote>%s</blockquote>' %
                                cgi.escape(expense_entry))
        	test = ExpenseEntry(expense.item, expense.amount, expense.paid_by)
